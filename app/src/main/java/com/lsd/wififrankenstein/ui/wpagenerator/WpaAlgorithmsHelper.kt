...
class WpaAlgorithmsHelper(private val context: Context) {
    ...
    companion object {
        ...
        private val THOMSON_CHAR0 = byteArrayOf(
            '3'.code.toByte(), '3'.code.toByte(), '3'.code.toByte(), '3'.code.toByte(),
            '3'.code.toByte(), '3'.code.toByte(), '3'.code.toByte(), '3'.code.toByte(),
            '3'.code.toByte(), '3'.code.toByte(), '4'.code.toByte(), '4'.code.toByte(),
        ...
        private val ARCADYAN_REGEX_1 = Regex("(Arcor|EasyBox|Vodafone|WLAN)(-| )[0-9a-fA-F]{6}")
        private val ARCADYAN_REGEX_2 = Regex("Vodafone[0-9a-zA-Z]{4}")
        private val SERCOMM_REGEX = Regex("SerComm[0-9a-fA-F]{6}")
        private val ACTIONTEC_REGEX = Regex("Actiontec[0-9a-fA-F]{6}")
        private val NETCOMM_REGEX = Regex("NetComm [0-9a-fA-F]{4}")
        private val BILLION_REGEX = Regex("Billion[0-9a-fA-F]{4}")
        private val SMC_REGEX = Regex("SMC[0-9a-fA-F]{6}")
        private val MOTOROLA_REGEX = Regex("Motorola[0-9a-fA-F]{6}")
        private val ARRIS_REGEX = Regex("ARRIS-[0-9a-fA-F]{4}")
        private val BROADCOM_REGEX = Regex("Broadcom[0-9a-fA-F]{6}")
        private val FREEBOX_REGEX = Regex("Freebox-[0-9a-fA-F]{6}")
        private val SFR_REGEX = Regex("SFR_[0-9a-fA-F]{4}")
        private val TELSTRA_REGEX = Regex("Telstra[0-9a-fA-F]{6}")
        private val SKYLINK_REGEX = Regex("Skylink_[0-9a-fA-F]{6}")
        private val OTE_REGEX = Regex("OTE[0-9a-fA-F]{6}")
        private val FRITZ_REGEX = Regex("FRITZ!Box [0-9]{4}")
        private val ZTE_REGEX = Regex("ZTE[0-9a-fA-F]{6}")
        private val TPLINK_REGEX = Regex("TP-LINK_[0-9A-F]{4}")
        private val TPLINK_LOWER_REGEX = Regex("TP-Link_[0-9A-F]{4}")
        private val MIKROTIK_REGEX = Regex("MikroTik-[0-9A-F]{6}")
        private val UBNT_REGEX = Regex("ubnt[0-9a-fA-F]{6}")
        private val ANDROID_AP_REGEX = Regex("AndroidAP.*")
        private val ANDROID_REGEX = Regex("Android_[0-9A-F]{4}")
        private val IPHONE_REGEX = Regex(".*'s iPhone")
        private val HUAWEI_5G_REGEX = Regex("HUAWEI-[0-9A-F]{4}-5G")
        private val HUAWEI_5G_LOWER_REGEX = Regex("HUAWEI_[0-9A-F]{4}_5G")
        private val SPEEDPORT_REGEX = Regex("Speedport_W [0-9]{3}[A-Z]")
        private val OPERA_REGEX = Regex("OPERA_[0-9A-F]{6}")
        private val WIND3_REGEX = Regex("Wind3 HuaweiMobile-[0-9A-F]{4}")
        private val BELKIN_REGEX = Regex("^(B|b)elkin(\\.|_)[0-9a-fA-F]{3,6}$")
        private val ALICE_REGEX = Regex("ALICE-WLAN[0-9a-fA-F]{2}")
        private val INFINITUM_REGEX = Regex("INFINITUM[0-9a-zA-Z]{4}")
        private val DISCUS_REGEX = Regex("Discus--?[0-9a-fA-F]{6}")
        private val CONN_X_REGEX = Regex("conn-x[0-9a-fA-F]{6}")
        private val EIRCOM_REGEX = Regex("eircom[0-1]{4} [0-1]{4}")
        private val D_LINK_REGEX = Regex("(DL|dl)ink-[0-9a-fA-F]{6}")
        private val P1_REGEX = Regex("[pP]1[0-9]{6}0000[0-9]")
        private val ADSLPT_REGEX = Regex("ADSLPT-AB[0-9]{5}")
        private val UPC_7_REGEX = Regex("UPC[0-9]{7}")
        private val UPC_56_REGEX = Regex("UPC[0-9]{5,6}")
        private val UPC_8_REGEX = Regex("UPC[0-9]{8}")
        private val NETGEAR_REGEX = Regex("NETGEAR[0-9]{2}")
        private val LINKSYS_REGEX = Regex("Linksys[0-9a-fA-F]{5}")
        private val ASUS_REGEX = Regex("ASUS[_-][0-9a-fA-F]{2}")
        private val BUFFALO_REGEX = Regex("Buffalo-[AG]-[0-9a-fA-F]{4}")
        private val LIVEBOX_REGEX = Regex("Livebox-[0-9a-fA-F]{4}")
        private val BBOX_REGEX = Regex("bbox[2-3]-[0-9a-fA-F]{4}")
        private val TECHNICOLOR_REGEX = Regex("Technicolor[0-9a-fA-F]{4}")
        private val SAGEM_FAST_REGEX = Regex("FAST[0-9]{4}")
        private val VODAFONE_WIFI_REGEX = Regex("VodafoneWiFi[0-9a-fA-F]{6}")
        private val VODAFONE_4_REGEX = Regex("Vodafone[0-9a-zA-Z]{4}")
        private val BT_HOME_HUB_REGEX = Regex("BTHomeHub[2-5]-[0-9A-Z]{4}")
        private val MOVISTAR_REGEX = Regex("Movistar_[0-9a-fA-F]{4}")
        private val JAZZTEL_REGEX = Regex("JAZZTEL_[0-9a-fA-F]{4}")
        private val HUAWEI_E5_REGEX = Regex("HUAWEI-E5[0-9]{2}-[0-9a-fA-F]{4}")
        private val E5_REGEX = Regex("E5[0-9]{3}-[0-9a-fA-F]{4}")
        private val EE_REGEX = Regex("EE-[0-9a-zA-Z]{6}")
        private val THOMSON_REGEX =
            Regex("(Thomson|SpeedTouch|Orange|INFINITUM|O2Wireless|Bbox|Alice-|DMAX|privat|TN_private|BigPond|CytaHome)[0-9a-fA-F]{6}")
        private val HEX_SPLIT_REGEX = Regex("[A-Fa-f]")
        private val ARCADYAN_ORDERS = arrayOf(
            intArrayOf(6, 2, 3, 8, 5, 1, 7, 4),
            intArrayOf(1, 2, 3, 8, 5, 1, 7, 4),
            intArrayOf(1, 2, 3, 8, 5, 6, 7, 4),
            intArrayOf(6, 2, 3, 8, 5, 6, 7, 4)
        )
        private val ARCADYAN_CHARSETS = arrayOf("024613578ACE9BDF", "944626378ace9bdf")
        private val SITECOM_CHARSETS341 =
            arrayOf("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ", "W0X1CDYNJU8VOZA0BKL46PQ7RS9T2E5HI3MFG")
        private val SITECOM_CHARSETS4000 =
            arrayOf("23456789ABCDEFGHJKLMNPQRSTUVWXYZ38BZ", "WXCDYNJU8VZABKL46PQ7RS9T2E5H3MFGPWR2")
        private val SITECOM_CHARSETS4004 =
            arrayOf("JKLMNPQRST23456789ABCDEFGHUVWXYZ38BK", "E5MFJUWXCDKL46PQHAB3YNJ8VZ7RS9TR2GPW")
        private val IPHONE_WORDS =
            listOf("apple", "iphone", "ipad", "wifi", "password", "123456", "12345678")
        private val IPHONE_NUMBERS =
            listOf("123", "456", "789", "000", "111", "2020", "2021", "2022", "2023", "2024")
    }
    ...
    private var loadedThomsonRaw: ByteArray? = null
    private var loadedTele2: List<TeleTuMagicInfo>? = null

    private val generateCache = LruCache<String, List<WpaResult>>(100)

    private val loadScope = CoroutineScope(Dispatchers.IO + SupervisorJob())

    private val cachedAlgorithms: List<WpaAlgorithm> by lazy {
        getAllAlgorithms()
    }
    ...
    fun generateKeys(ssid: String, bssid: String): List<WpaResult> {
        ...
        val cacheKey = "$cleanSsid|$cleanBssid"

        generateCache[cacheKey]?.let { return it }
        ...
        val sortedResults = results.sortedByDescending { it.supportState }
        generateCache[cacheKey] = sortedResults
    ...
    private suspend fun loadResources() {
        try {
                ...
                ZipInputStream(input).use { zip ->
                    ...
                    while (entry != null) {
                        if (entry.size > 10 * 1024 * 1024) {
                            Log.w(
                                "WpaAlgorithmsHelper",
                                "Skipping large magic file: ${entry.name} (${entry.size} bytes)"
                            )
                            entry = zip.nextEntry
                            continue
                        }
                        ...
                        when (entry.name) {
                            ...
                            "cyta_bases.txt" -> loadedCyta = parseCytaMagicInfo(content)
                            "cyta_zte_bases.txt" -> loadedCytaZTEs = parseCytaZTEMagicInfo(content)
                            "netfaster_bases.txt" -> loadedNetfasters =
                                parseNetfasterMagicInfo(content)
            ...
            try {
                ...
                if (routerKeygenFile.exists() && routerKeygenFile.length() < 100 * 1024 * 1024) {
                    loadedThomsonRaw = routerKeygenFile.readBytes()
                }
            } catch (e: Exception) {
    ...
    private fun parseAliceMagicInfo(content: ByteArray): List<AliceMagicInfo> {
        return content.toString(Charsets.UTF_8).lines().mapNotNull {
            ...
            if (parts.size == 5) {
                try {
                    AliceMagicInfo(parts[1], intArrayOf(parts[2].toInt(), parts[3].toInt()))
    ...
    private fun parseCytaMagicInfo(content: ByteArray): List<CytaMagicInfo> {
        return content.toString(Charsets.UTF_8).lines().mapNotNull {
            ...
            if (parts.size == 4) {
                try {
                    CytaMagicInfo(parts[1], parts[2].toLong(16), parts[3].toLong(16))
                } catch (e: NumberFormatException) {
                    null
                }
            } else null
    ...
    private fun parseCytaZTEMagicInfo(content: ByteArray): Map<String, List<CytaMagicInfo>> {
        ...
        content.toString(Charsets.UTF_8).lines().forEach { line ->
            val parts = line.trim().split(" ")
            if (parts.size == 4) {
                try {
                    val base = parts[1].toLong()
                    val divider =
                        if (parts[2].equals("0", ignoreCase = true)) 4L else parts[2].toLong()
                    val info = CytaMagicInfo(parts[3], base, divider)
    ...
    private fun parseNetfasterMagicInfo(content: ByteArray): List<NetfasterMagicInfo> {
        return content.toString(Charsets.UTF_8).lines().mapNotNull {
            ...
            if (parts.size == 4) {
                try {
                    val base = parts[1].toInt()
                    val divisors = if (parts[2].equals("0", ignoreCase = true)) intArrayOf(
                        4,
                        5,
                        9
                    ) else intArrayOf(parts[2].toInt())
                    NetfasterMagicInfo(parts[0], base, divisors, parts[3])
                } catch (e: NumberFormatException) {
                    null
                }
            } else null
    ...
    private fun parseOteHuaweiMagicValues(content: ByteArray): List<String> {
        return content.toString(Charsets.UTF_8).lines().firstOrNull()?.split(" ")
    ...
    private fun getAllAlgorithms(): List<WpaAlgorithm> {
        return listOf(
            ...
            ThomsonAlgorithm(loadedThomsonRaw),
            ComtrendAlgorithm(),
            AndaredAlgorithm(),
    ...
    data class NetfasterMagicInfo(
        val mac: String,
        val base: Int,
        val divisors: IntArray,
        val key: String
    )
    ...
    class ArcadyanAlgorithm : WpaAlgorithm() {
        override fun getName() = "Arcadyan"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(ARCADYAN_REGEX_1) ||
                        ssid.matches(ARCADYAN_REGEX_2) -> SUPPORTED
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            return try {
                ...
                val orders = ARCADYAN_ORDERS
                val charsets = ARCADYAN_CHARSETS
    ...
    class SercommAlgorithm : WpaAlgorithm() {
    ...
    class ActiontecAlgorithm : WpaAlgorithm() {
    ...
    class NetcommAlgorithm : WpaAlgorithm() {
    ...
    class BillionAlgorithm : WpaAlgorithm() {
    ...
    class SMCAlgorithm : WpaAlgorithm() {
    ...
    class MotorolaAlgorithm : WpaAlgorithm() {
    ...
    class ArrisAlgorithm : WpaAlgorithm() {
    ...
    class GemtekAlgorithm : WpaAlgorithm() {
    ...
    class BroadcomAlgorithm : WpaAlgorithm() {
    ...
    class RealtekAlgorithm : WpaAlgorithm() {
    ...
    class FreeboxAlgorithm : WpaAlgorithm() {
        override fun getName() = "Freebox"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(FREEBOX_REGEX) -> SUPPORTED
    ...
    class SFRAlgorithm : WpaAlgorithm() {
        override fun getName() = "SFR"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(SFR_REGEX) -> SUPPORTED
    ...
    class TelstraAlgorithm : WpaAlgorithm() {
        override fun getName() = "Telstra"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(TELSTRA_REGEX) -> SUPPORTED
    ...
    class SkylinkAlgorithm : WpaAlgorithm() {
        override fun getName() = "Skylink"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(SKYLINK_REGEX) -> SUPPORTED
    ...
    class OteGenericAlgorithm : WpaAlgorithm() {
        override fun getName() = "OteGeneric"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(OTE_REGEX)) SUPPORTED else UNLIKELY_SUPPORTED
        }
    ...
    class FritzBoxAlgorithm : WpaAlgorithm() {
        override fun getName() = "FritzBox"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(FRITZ_REGEX) -> SUPPORTED
    ...
    class ZTEAlgorithm : WpaAlgorithm() {
        override fun getName() = "ZTE"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(ZTE_REGEX) -> SUPPORTED
    ...
    class TpLinkNewAlgorithm : WpaAlgorithm() {
        override fun getName() = "TpLinkNew"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(TPLINK_REGEX) -> SUPPORTED
                ssid.matches(TPLINK_LOWER_REGEX) -> SUPPORTED
    ...
    class MikroTikAlgorithm : WpaAlgorithm() {
        override fun getName() = "MikroTik"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(MIKROTIK_REGEX) -> SUPPORTED
    ...
    class UbiquitiAlgorithm : WpaAlgorithm() {
        override fun getName() = "Ubiquiti"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(UBNT_REGEX) -> SUPPORTED
    ...
    class AndroidHotspotAlgorithm : WpaAlgorithm() {
        override fun getName() = "AndroidHotspot"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(ANDROID_AP_REGEX) -> SUPPORTED
                ssid.matches(ANDROID_REGEX) -> SUPPORTED
    ...
    class IPhoneHotspotAlgorithm : WpaAlgorithm() {
        ...
        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ...
                ssid.matches(IPHONE_REGEX) -> SUPPORTED
                else -> UNSUPPORTED
            }
        }
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            val words = IPHONE_WORDS
            val numbers = IPHONE_NUMBERS
    ...
    class Huawei5GAlgorithm : WpaAlgorithm() {
        override fun getName() = "Huawei5G"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(HUAWEI_5G_REGEX) -> SUPPORTED
                ssid.matches(HUAWEI_5G_LOWER_REGEX) -> SUPPORTED
    ...
    class Speedport2Algorithm : WpaAlgorithm() {
        override fun getName() = "Speedport2"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(SPEEDPORT_REGEX) -> SUPPORTED
    ...
    class OperaAlgorithm : WpaAlgorithm() {
        override fun getName() = "Opera"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(OPERA_REGEX) -> SUPPORTED
    ...
    class Wind3Algorithm : WpaAlgorithm() {
        override fun getName() = "Wind3"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(WIND3_REGEX) -> SUPPORTED
    ...
    class BelkinAlgorithm : WpaAlgorithm() {
        override fun getName() = "Belkin"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(BELKIN_REGEX)) SUPPORTED else UNSUPPORTED
        }
    ...
    class ArnetPirelliAlgorithm : WpaAlgorithm() {
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            try {
                ...
                for (i in 0 until 12 step 2) {
                    macBytes[i / 2] =
                        ((newMac[i].digitToInt(16) shl 4) + newMac[i + 1].digitToInt(16)).toByte()
                }
                md.reset()
                md.update(aliceSeed)
    ...
    class AliceGermanyAlgorithm : WpaAlgorithm() {
        override fun getName() = "AliceGermany"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(ALICE_REGEX)) SUPPORTED else UNSUPPORTED
        }
    ...
    class CabovisaoSagemAlgorithm : WpaAlgorithm() {
    ...
    class HuaweiAlgorithm : WpaAlgorithm() {
        override fun getName() = "Huawei"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(INFINITUM_REGEX)) SUPPORTED else UNSUPPORTED
        }
    ...
    class InfostradaAlgorithm : WpaAlgorithm() {
    ...
    class CytaZTEAlgorithm(private val cytaZTEs: Map<String, List<CytaMagicInfo>>?) :
        WpaAlgorithm() {
    ...
    class InterCableAlgorithm : WpaAlgorithm() {
    ...
    class CytaAlgorithm(private val cyta: List<CytaMagicInfo>?) : WpaAlgorithm() {
        override fun getName() = "Cyta"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.startsWith("CYTA") || ssid.matches(DISCUS_REGEX)) SUPPORTED else UNSUPPORTED
        }
    ...
    class HG824xAlgorithm : WpaAlgorithm() {
    ...
    class ConnAlgorithm : WpaAlgorithm() {
        override fun getName() = "Conn"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(OTE_REGEX) -> SUPPORTED
                ssid.matches(CONN_X_REGEX) -> {
                    if (mac.length == 12) {
                        ...
                        ) SUPPORTED else UNLIKELY_SUPPORTED
                    } else UNSUPPORTED
                }
    ...
    class DiscusAlgorithm : WpaAlgorithm() {
        override fun getName() = "Discus"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(DISCUS_REGEX)) SUPPORTED else UNSUPPORTED
        }

        override fun generateKeys(ssid: String, mac: String): List<String> {
            if (!ssid.matches(DISCUS_REGEX)) return emptyList()
    ...
    class EircomAlgorithm : WpaAlgorithm() {
        override fun getName() = "Eircom"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(EIRCOM_REGEX)) SUPPORTED else UNSUPPORTED
        }
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            try {
                ...
                for (i in 0 until 6 step 2) {
                    routerMAC[i / 2 + 1] =
                        ((macPart[i].digitToInt(16) shl 4) + macPart[i + 1].digitToInt(16)).toByte()
                }
    ...
    class DlinkAlgorithm : WpaAlgorithm() {
        override fun getName() = "Dlink"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(D_LINK_REGEX)) SUPPORTED else UNSUPPORTED
        }
    ...
    class OnoAlgorithm : WpaAlgorithm() {
        override fun getName() = "Ono"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(P1_REGEX)) SUPPORTED else UNSUPPORTED
        }
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            try {
                ...
                var randNumber =
                    pseed[0] or (pseed[1] shl 8) or (pseed[2] shl 16) or (pseed[3] shl 24)
    ...
    class OteBAUDAlgorithm : WpaAlgorithm() {
        override fun getName() = "OteBAUD"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(OTE_REGEX) && !ssid.startsWith("CYTA")) SUPPORTED else UNSUPPORTED
        }
    ...
    class MeoPirelliAlgorithm : WpaAlgorithm() {
        override fun getName() = "MeoPirelli"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(ADSLPT_REGEX)) SUPPORTED else UNSUPPORTED
        }
    ...
    class NetFasterAlgorithm(private val netfasters: List<NetfasterMagicInfo>?) :
        WpaAlgorithm() {
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            for (info in netfasters) {
                for (div in info.divisors) {
                    val basi = info.mac.toLong(16) - info.base.toLong() * div
                    val diff = macDec - basi
                    if (diff >= 0 && diff <= (9999L * div) && diff % div == 0L) {
    ...
    class TeleTuAlgorithm(private val tele2: List<TeleTuMagicInfo>?) : WpaAlgorithm() {
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            for (magicInfo in tele2) {
                val serial =
                    ((mac.substring(6).toInt(16) - magicInfo.base) / magicInfo.divider).toString()
                        .padStart(7, '0')
    ...
    class Speedport500Algorithm : WpaAlgorithm() {
    ...
    class SitecomX500Algorithm : WpaAlgorithm() {
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            fun generateKey(macInput: String) {
                ...
                val numericMac =
                    "0${macInput.substring(6).split(HEX_SPLIT_REGEX)[0]}".toIntOrNull() ?: return
    ...
    class SitecomWLR341Algorithm : WpaAlgorithm() {
        override fun getName() = "SitecomWLR341"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.lowercase()
                    .equals("sitecom${mac.substring(6)}", ignoreCase = true)
            ) SUPPORTED else UNLIKELY_SUPPORTED
        }
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            val results = mutableListOf<String>()
            val charsets341 = SITECOM_CHARSETS341
            val charsets4000 = SITECOM_CHARSETS4000
            val charsets4004 = SITECOM_CHARSETS4004
    ...
    class Sitecom2100Algorithm : WpaAlgorithm() {
    ...
    class UpcAlgorithm : WpaAlgorithm() {
        ...
        private fun generateUpcKeys(ssid: String): List<String> {
            ...
            for (year in 4..9) {
                val yearBase = year * 36 * 36 * 36 * 52
                val remainder = ssidNum - yearBase

                if (remainder < 0) continue

                val week = remainder / (36 * 36 * 36)
                val weekRemainder = remainder % (36 * 36 * 36)

                if (week !in 1..53) continue

                val a = weekRemainder / (36 * 36)
                val aRemainder = weekRemainder % (36 * 36)
                val b = aRemainder / 36
                val c = aRemainder % 36

                val key = "CP${year}${
                    week.toString().padStart(2, '0')
                }${intToChar(a)}${intToChar(b)}${intToChar(c)}"
                results.add(key)
            }
            return results
        }
    ...
    class AlcatelLucentAlgorithm : WpaAlgorithm() {
    ...
    class PtvAlgorithm : WpaAlgorithm() {
    ...
    class NetgearAlgorithm : WpaAlgorithm() {
        override fun getName() = "Netgear"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(NETGEAR_REGEX) -> SUPPORTED
    ...
    class LinksysAlgorithm : WpaAlgorithm() {
        override fun getName() = "Linksys"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(LINKSYS_REGEX) -> SUPPORTED
    ...
    class AsusAlgorithm : WpaAlgorithm() {
        override fun getName() = "Asus"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(ASUS_REGEX) -> SUPPORTED
    ...
    class BuffaloAlgorithm : WpaAlgorithm() {
        override fun getName() = "Buffalo"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(BUFFALO_REGEX) -> SUPPORTED
    ...
    class LiveboxAlgorithm : WpaAlgorithm() {
        override fun getName() = "Livebox"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(LIVEBOX_REGEX) -> SUPPORTED
    ...
    class BboxAlgorithm : WpaAlgorithm() {
        override fun getName() = "Bbox"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(BBOX_REGEX) -> SUPPORTED
    ...
    class TechnicolorAlgorithm : WpaAlgorithm() {
        override fun getName() = "Technicolor"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(TECHNICOLOR_REGEX) -> SUPPORTED
    ...
    class SagemcomAlgorithm : WpaAlgorithm() {
        override fun getName() = "Sagemcom"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(SAGEM_FAST_REGEX) -> SUPPORTED
    ...
    class VodafoneAlgorithm : WpaAlgorithm() {
        override fun getName() = "VodafoneGeneric"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(VODAFONE_WIFI_REGEX) -> SUPPORTED
                ssid.startsWith("Vodafone") && !ssid.matches(VODAFONE_4_REGEX) -> UNLIKELY_SUPPORTED
                else -> UNSUPPORTED
            }
        }
    ...
    class BTHomeHubAlgorithm : WpaAlgorithm() {
        override fun getName() = "BTHomeHub"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(BT_HOME_HUB_REGEX) -> SUPPORTED
    ...
    class MovistarAlgorithm : WpaAlgorithm() {
        override fun getName() = "Movistar"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(MOVISTAR_REGEX) -> SUPPORTED
    ...
    class JazztelAlgorithm : WpaAlgorithm() {
        override fun getName() = "Jazztel"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(JAZZTEL_REGEX) -> SUPPORTED
    ...
    class HuaweiE5Algorithm : WpaAlgorithm() {
        override fun getName() = "HuaweiE5"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(HUAWEI_E5_REGEX) -> SUPPORTED
                ssid.matches(E5_REGEX) -> SUPPORTED
    ...
    class EEAlgorithm : WpaAlgorithm() {
        override fun getName() = "EE"

        override fun getSupportState(ssid: String, mac: String): Int {
            return when {
                ssid.matches(EE_REGEX) -> SUPPORTED
    ...
    class MegaredAlgorithm : WpaAlgorithm() {
    ...
    class MaxcomAlgorithm : WpaAlgorithm() {
    ...
    class OteHuaweiAlgorithm(private val oteHuawei: List<String>?) : WpaAlgorithm() {
    ...
    class ThomsonAlgorithm(private val thomsonRaw: ByteArray?) : WpaAlgorithm() {
        override fun getName() = "Thomson"

        override fun getSupportState(ssid: String, mac: String): Int {
            return if (ssid.matches(THOMSON_REGEX)) SUPPORTED else UNSUPPORTED
        }
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            if (thomsonRaw != null) {
                try {
                    val md = MessageDigest.getInstance("SHA1")
                    val routerESSID = ByteArray(3)
                    for (i in 0 until 6 step 2) {
                        routerESSID[i / 2] =
                            ((ssidIdentifier[i].digitToInt(16) shl 4) + ssidIdentifier[i + 1].digitToInt(
                                16
                            )).toByte()
                    }
                    val keys = parseBinaryDic(thomsonRaw, routerESSID, md)
                    results.addAll(keys)
                } catch (e: Exception) {
                    Log.w("ThomsonAlgorithm", "Binary dic parse failed, fallback to MD5", e)
                }
            }
            ...
            if (results.isEmpty()) {
                try {
                    ...
                    for (i in 0 until 12 step 2) {
                        macBytes[i / 2] =
                            ((mac[i].digitToInt(16) shl 4) + mac[i + 1].digitToInt(16)).toByte()
                    }
                    md.reset()
                    md.update(macBytes)
                    md.update(ssidIdentifier.toByteArray(Charsets.UTF_8))
                    val hash = md.digest()
                    val key = buildString {
                        repeat(10) {
                            append((hash[it].toInt() and 0xFF).toString(16).padStart(2, '0'))
        ...
        private fun parseBinaryDic(
            data: ByteArray,
            routerESSID: ByteArray,
            md: MessageDigest
        ): List<String> {
            if (data.size < 1282) return emptyList()

            val version = ((data[0].toInt() and 0xFF) shl 8) or (data[1].toInt() and 0xFF)
            if (version !in 1..2 && version != 4) return emptyList()

            val essid0 = routerESSID[0].toInt() and 0xFF
            val essid1 = routerESSID[1].toInt() and 0xFF

            val l1Offset = essid0 * 5 + 2
            var offset = 0
            var lastLength = 0

            if (l1Offset + 5 <= data.size && data[l1Offset] == routerESSID[0]) {
                offset = ((data[l1Offset + 1].toInt() and 0xFF) shl 24) or
                        ((data[l1Offset + 2].toInt() and 0xFF) shl 16) or
                        ((data[l1Offset + 3].toInt() and 0xFF) shl 8) or
                        (data[l1Offset + 4].toInt() and 0xFF)
                if (essid0 != 0xFF && l1Offset + 10 <= data.size) {
                    lastLength = ((data[l1Offset + 6].toInt() and 0xFF) shl 24) or
                            ((data[l1Offset + 7].toInt() and 0xFF) shl 16) or
                            ((data[l1Offset + 8].toInt() and 0xFF) shl 8) or
                            (data[l1Offset + 9].toInt() and 0xFF)
                }
            }

            var totalOffset = offset
            if (totalOffset + 1024 > data.size) return emptyList()

            val l2Offset = totalOffset + essid1 * 4
            var length = 0

            if (data[l2Offset] == routerESSID[1]) {
                offset = ((data[l2Offset + 1].toInt() and 0xFF) shl 16) or
                        ((data[l2Offset + 2].toInt() and 0xFF) shl 8) or
                        (data[l2Offset + 3].toInt() and 0xFF)
                length = ((data[l2Offset + 5].toInt() and 0xFF) shl 16) or
                        ((data[l2Offset + 6].toInt() and 0xFF) shl 8) or
                        (data[l2Offset + 7].toInt() and 0xFF)
            }

            totalOffset += offset
            length -= offset
            if (lastLength != 0 && essid1 == 0xFF) {
                lastLength -= totalOffset
                length = lastLength
            }
            if (essid0 == 0xFF && essid1 == 0xFF) {
                length = 2000
            }

            if (totalOffset + length > data.size) return emptyList()
            val entry = data.copyOfRange(totalOffset, totalOffset + length)
            ...
            when (version) {
                1 -> {
                    var off = 0
                    while (off + 4 <= entry.size) {
                        if (entry[off] == routerESSID[2]) {
                            val seq = ((entry[off + 1].toInt() and 0xFF) shl 16) or
                                    ((entry[off + 2].toInt() and 0xFF) shl 8) or
                                    (entry[off + 3].toInt() and 0xFF)
                            val key = computeThomsonKey(seq, cp, md, routerESSID)
                ...
                2 -> {
                    var off = 0
                    while (off + 3 <= entry.size) {
                        val seq = ((entry[off].toInt() and 0xFF) shl 16) or
                                ((entry[off + 1].toInt() and 0xFF) shl 8) or
                                (entry[off + 2].toInt() and 0xFF)
                        val key = computeThomsonKey(seq, cp, md, routerESSID)
                        if (key != null) results.add(key)
                        off += 3
                    }
                ...
                4 -> {
                    var off = 0
                    while (off + 3 <= entry.size) {
                        val baseSeq = ((entry[off].toInt() and 0xFF) shl 16) or
                                ((entry[off + 1].toInt() and 0xFF) shl 8) or
                                (entry[off + 2].toInt() and 0xFF)
                        for (i in 0..1) {
                            val seq = i + baseSeq * 2
        ...
        private fun computeThomsonKey(
            sequenceNumber: Int,
            cp: ByteArray,
            md: MessageDigest,
            routerESSID: ByteArray
        ): String? {
            val c = sequenceNumber % 36
            val b = sequenceNumber / 36 % 36
            val a = sequenceNumber / (36 * 36) % 36
            val year = sequenceNumber / (36 * 36 * 36 * 52) + 4
            val week = (sequenceNumber / (36 * 36 * 36)) % 52 + 1

            cp[2] = ('0'.code + (year / 10)).toByte()
            cp[3] = ('0'.code + (year % 10)).toByte()
            cp[4] = ('0'.code + (week / 10)).toByte()
            cp[5] = ('0'.code + (week % 10)).toByte()
            cp[6] = THOMSON_CHAR0[a]
            cp[7] = THOMSON_CHAR1[a]
            cp[8] = THOMSON_CHAR0[b]
            cp[9] = THOMSON_CHAR1[b]
            cp[10] = THOMSON_CHAR0[c]
            cp[11] = THOMSON_CHAR1[c]

            md.reset()
            md.update(cp)
            val hash = md.digest()

            if (hash[19] != routerESSID[2] || hash[18] != routerESSID[1] || hash[17] != routerESSID[0]) return null

            return hash.toHexString().substring(0, 10).uppercase()
        }
    }

    class ComtrendAlgorithm : WpaAlgorithm() {
    ...
    class AndaredAlgorithm : WpaAlgorithm() {
    ...
    class AliceItalyAlgorithm(private val alice: List<AliceMagicInfo>?) : WpaAlgorithm() {
            ...
            try {
                ...
                for (aliceInfo in alice) {
                    ...
                    if (macEth != mac.substring(0, 6)) {
                        for (i in 0 until 12 step 2) {
                            macBytes[i / 2] =
                                ((macEth[i].digitToInt(16) shl 4) + macEth[i + 1].digitToInt(16)).toByte()
                        }
                        md.reset()
                        md.update(aliceSeed)
                        md.update(serialStr.toByteArray(Charsets.UTF_8))
    ...
    class AxtelAlgorithm : WpaAlgorithm() {
    ...
    class TecomAlgorithm : WpaAlgorithm() {
    ...
    class TplinkAlgorithm : WpaAlgorithm() {
    ...
    class ZyxelAlgorithm : WpaAlgorithm() {
    ...
    class SkyV1Algorithm : WpaAlgorithm() {
    ...
    class PirelliAlgorithm : WpaAlgorithm() {
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            try {
                ...
                key[1] =
                    (((hash[0].toInt() and 0x07) shl 2) or ((hash[1].toInt() and 0xC0) shr 6)).toShort()
                key[2] = ((hash[1].toInt() and 0x3E) shr 1).toShort()
                key[3] =
                    (((hash[1].toInt() and 0x01) shl 4) or ((hash[2].toInt() and 0xF0) shr 4)).toShort()
                key[4] =
                    (((hash[2].toInt() and 0x0F) shl 1) or ((hash[3].toInt() and 0x80) shr 7)).toShort()
    ...
    class VerizonAlgorithm : WpaAlgorithm() {
    ...
    class WifimediaRAlgorithm : WpaAlgorithm() {
    ...
    class PBSAlgorithm : WpaAlgorithm() {
    ...
    class Wlan2Algorithm : WpaAlgorithm() {
    ...
    class Wlan6Algorithm : WpaAlgorithm() {
        ...
        override fun generateKeys(ssid: String, mac: String): List<String> {
            ...
            for (i in 0 until 10) {
                val aux =
                    i + (ssidSubPart[3].code and 0xf) + (bssidLastByte[0].code and 0xf) + (bssidLastByte[1].code and 0xf)
                val aux1 =
                    (ssidSubPart[1].code and 0xf) + (ssidSubPart[2].code and 0xf) + (ssidSubPart[4].code and 0xf) + (ssidSubPart[5].code and 0xf)
    ...
    class JenkinsHash {
        ...
        private fun rot(val1: Long, shift: Int): Long =
            (leftShift(val1, shift) or (val1 ushr (32 - shift))) and maxValue
