...
class WiFiSpectrumView @JvmOverloads constructor(
    ...
    private var cachedChannelRange = listOf<Int>()
    private var cachedMinChannel = 1
    private var cachedMaxChannel = 11
    private var cachedChannelSpan = 10
    ...
    fun updateSpectrumData(
        networks: List<ScanResult>,
        excludedBssids: Set<String>,
        band: FrequencyBand
    ) {
        this.networkInfoList = networks
            ...
            .mapNotNull { result ->
                val channel =
                    NetworkFrequencyAnalyzer.determineChannelFromFrequency(result.frequency, band)
                if (channel != -1) {
        ...
        updateCachedChannelRange()
        invalidate()
    }

    private fun updateCachedChannelRange() {
        cachedChannelRange = getChannelRangeForBand()
        cachedMinChannel = cachedChannelRange.minOrNull() ?: 1
        cachedMaxChannel = cachedChannelRange.maxOrNull() ?: 11
        cachedChannelSpan =
            if (cachedMaxChannel - cachedMinChannel == 0) 1 else cachedMaxChannel - cachedMinChannel
    }
    ...
    private fun drawEmptyMessage(canvas: Canvas) {
        ...
        canvas.drawText(
            context.getString(R.string.no_wifi_data_available),
            centerX,
            centerY,
            textRenderPaint
        )
    ...
    private fun drawGridLines(canvas: Canvas, drawableWidth: Float, drawableHeight: Float) {
        val step = when (currentBand) {
            FrequencyBand.GHZ_2_4 -> 1
            FrequencyBand.GHZ_5 -> 4
            FrequencyBand.GHZ_6 -> 4
        }

        for (i in cachedChannelRange.indices step step) {
            val channel = cachedChannelRange[i]
            val x =
                canvasPadding + (channel - cachedMinChannel).toFloat() / cachedChannelSpan * drawableWidth
    ...
    private fun drawAxes(canvas: Canvas, drawableWidth: Float, drawableHeight: Float) {
        ...
        canvas.drawLine(
            canvasPadding,
            canvasPadding + drawableHeight,
            canvasPadding + drawableWidth,
            canvasPadding + drawableHeight,
            backgroundPaint
        )
        canvas.drawLine(
    ...
    private fun drawSpectrumData(canvas: Canvas, drawableWidth: Float, drawableHeight: Float) {
        ...
        networkInfoList.forEach { networkInfo ->
            ...
            if (channel in cachedMinChannel..cachedMaxChannel) {
                val centerX =
                    canvasPadding + (channel - cachedMinChannel).toFloat() / cachedChannelSpan * drawableWidth
                val signalLevel =
                    max(minSignalLevel, min(maxSignalLevel, networkInfo.scanResult.level))
    ...
    private fun renderNetworkSpectrum(
        ...
        val widthSpread = when (currentBand) {
            FrequencyBand.GHZ_2_4 -> {
                ...
                (channelsSpread / cachedChannelSpan) * drawableWidth
            }
            ...
            FrequencyBand.GHZ_5, FrequencyBand.GHZ_6 -> {
                val channelsSpread = when (bandwidth) {
                    ...
                    ChannelBandwidth.WIDTH_80_PLUS_80 -> 8.0f
                    ChannelBandwidth.WIDTH_160 -> 8.0f
                    ChannelBandwidth.WIDTH_320 -> 16.0f
                }
                (channelsSpread / cachedChannelRange.size) * drawableWidth
            }
        }
        ...
        if (rightX - leftX > 60 && networkInfo.scanResult.SSID.isNotBlank()) {
            ...
            val labelY = max(peakY - 16, canvasPadding + 20)
            canvas.drawText(ssidText, centerX, labelY, textRenderPaint)
    ...
    private fun drawAxisLabels(canvas: Canvas, drawableWidth: Float, drawableHeight: Float) {
        ...
        val stepSize = max(1, cachedChannelSpan / 10)
        for (i in cachedChannelRange.indices step stepSize) {
            val channel = cachedChannelRange[i]
            val x =
                canvasPadding + (channel - cachedMinChannel).toFloat() / cachedChannelSpan * drawableWidth
            canvas.drawText(
                channel.toString(),
                x,
                canvasPadding + drawableHeight + 40f,
                textRenderPaint
            )
        }
