...
sealed class API3WiFiMethodParams {
    ...
    class ApiQuery : API3WiFiMethodParams() {
        ...
        private fun setupInputFilters() {
            bssidInput.filters = arrayOf(InputFilter { source, start, end, dest, dstart, dend ->
                ...
                if (source.isNotEmpty() && newText.length == dstart + 1 && (dstart == 2 || dstart == 5 || dstart == 8 || dstart == 11 || dstart == 14)) {
        ...
        private fun updateViewsVisibility() {
            ...
            pairsList.visibility = View.VISIBLE
        }

        private fun addBssid() {
            val bssid = bssidInput.text.toString().trim().uppercase()
            if (validateBssidInput(bssid)) {
        ...
        private fun addEssid() {
            val essid = essidInput.text.toString().trim()
            if (validateEssidInput(essid)) {
        ...
        private fun addExactPair() {
            ...
            if (!validateBssidInput(bssid)) return
            if (!validateEssidInput(essid)) return
        ...
        private fun validateBssidInput(bssid: String): Boolean {
            if (bssid.isEmpty()) {
            ...
            if (!bssid.matches(Regex("^([0-9A-F]{2}:){5}[0-9A-F]{2}$"))) {
                bssidInputLayout.error =
                    bssidInputLayout.context.getString(R.string.invalid_bssid_format)
                return false
            }
        ...
        private fun validateEssidInput(essid: String): Boolean {
            if (essid.isEmpty()) {
        ...
        override fun getRequest(key: String): API3WiFiRequest? {
            return if (isValid()) {
                when (_currentQueryType) {
                    QueryType.BSSID_ONLY -> API3WiFiRequest.ApiQuery(
                        key = key,
                        bssidList = _bssidList,
                        essidList = null,
                        sens = caseSensitiveSwitch.isChecked
                    )

                    QueryType.ESSID_ONLY -> API3WiFiRequest.ApiQuery(
                        key = key,
                        bssidList = listOf("*"),
                        essidList = _essidList,
                        sens = caseSensitiveSwitch.isChecked
                    )

                    QueryType.EXACT_MATCH -> API3WiFiRequest.ApiQuery(
                        key = key,
                        bssidList = _exactPairs.map { it.first },
                        essidList = _exactPairs.map { it.second },
                        sens = caseSensitiveSwitch.isChecked
                    )
                }
            } else null
            ...
            when (_currentQueryType) {
                ...
                QueryType.ESSID_ONLY -> {
                    if (_essidList.isEmpty()) {
                        essidInputLayout.error =
                            essidInputLayout.context.getString(R.string.at_least_one_essid)
                    } else {
                        essidInputLayout.error = null
                    }
                }
                ...
                QueryType.EXACT_MATCH -> {
                    if (_exactPairs.isEmpty()) {
                        bssidInputLayout.error =
                            bssidInputLayout.context.getString(R.string.at_least_one_pair)
                        essidInputLayout.error =
                            bssidInputLayout.context.getString(R.string.at_least_one_pair)
                    } else {
                        bssidInputLayout.error = null
            ...
            override fun onBindViewHolder(holder: ViewHolder, position: Int) {
                when (_currentQueryType) {
                    ...
                    QueryType.ESSID_ONLY -> {
                        ...
                        holder.essidText.visibility = View.VISIBLE
                        holder.essidText.text = _essidList[position]
                        holder.removeButton.setOnClickListener {
                            val pos = holder.adapterPosition
                            if (pos != RecyclerView.NO_POSITION) {
                                _essidList.removeAt(pos)
                                notifyItemRemoved(pos)
                                validate()
                                onDataChangedListener?.invoke()
                            }
    ...
    class GetPointParams : API3WiFiMethodParams() {
        private lateinit var pointIdInput: TextInputEditText
        private lateinit var pointIdInputLayout: TextInputLayout

        override fun createView(context: Context, container: ViewGroup): View {
            val view = LayoutInflater.from(context).inflate(
                R.layout.layout_api_getpoint_params,
                container,
                false
            )
            pointIdInput = view.findViewById(R.id.pointIdInput)
            pointIdInputLayout = view.findViewById(R.id.pointIdInputLayout)
            return view
        }

        override fun getRequest(key: String): API3WiFiRequest? {
            val id = pointIdInput.text.toString().toIntOrNull()
            return if (id != null) API3WiFiRequest.TrpcGetPoint(id = id) else null
        }

        override fun isValid(): Boolean {
            return pointIdInput.text.toString().toIntOrNull() != null
        }

        override fun validate() {
            if (pointIdInput.text.toString().toIntOrNull() == null) {
                pointIdInputLayout.error =
                    pointIdInputLayout.context.getString(R.string.invalid_point_id)
            } else {
                pointIdInputLayout.error = null
            }
        }

        override fun clear() {
            pointIdInput.text?.clear()
            pointIdInputLayout.error = null
        }
    }

    class SearchNetworksParams : API3WiFiMethodParams() {
        private lateinit var chipSearchByBssid: Chip
        private lateinit var chipSearchBySsid: Chip
        private lateinit var queryInput: TextInputEditText
        private lateinit var queryInputLayout: TextInputLayout
        private var isBssidMode: Boolean = true

        override fun createView(context: Context, container: ViewGroup): View {
            val view = LayoutInflater.from(context).inflate(
                R.layout.layout_api_searchnetworks_params,
                container,
                false
            )
            chipSearchByBssid = view.findViewById(R.id.chipSearchByBssid)
            chipSearchBySsid = view.findViewById(R.id.chipSearchBySsid)
        ...
        private fun updateHint() {
            queryInputLayout.hint = if (isBssidMode) {
                queryInputLayout.context.getString(R.string.enter_mac_address)
            } else {
                queryInputLayout.context.getString(R.string.enter_network_name)
            }
        }
        ...
        override fun getRequest(key: String): API3WiFiRequest? {
            ...
            return API3WiFiRequest.TrpcSearchNetworks(
                query = if (isBssidMode) q.uppercase() else q,
                type = if (isBssidMode) "bssid" else "ssid"
            )
        }

        override fun isValid(): Boolean {
            return queryInput.text.toString().trim().isNotEmpty()
        }

        override fun validate() {
            if (queryInput.text.toString().trim().isEmpty()) {
                queryInputLayout.error =
                    queryInputLayout.context.getString(R.string.error_empty_input)
            } else {
                queryInputLayout.error = null
            }
        }
    ...
    companion object {
        fun create(methodName: String): API3WiFiMethodParams {
            return when (methodName) {
                ...
                "apidev" -> ApiDev()
                "apiranges" -> ApiRanges()
                "getpoint" -> GetPointParams()
                "searchnetworks" -> SearchNetworksParams()
