...
class UserLocationManager(private val context: Context) : LocationListener {
    ...
    private var singleLegacyListener: LocationListener? = null
            ...
            locationCallback = object : LocationCallback() {
                override fun onLocationResult(locationResult: LocationResult) {
                    ...
                    if (location != null) {
                        Log.d(
                            TAG,
                            "Fused location updated: ${location.latitude}, ${location.longitude}"
                        )
        ...
        try {
            ...
            val listener = object : LocationListener {
                override fun onLocationChanged(location: Location) {
                    Log.d(
                        TAG,
                        "Single legacy location: ${location.latitude}, ${location.longitude}"
                    )
                    _userLocation.postValue(GeoPoint(location.latitude, location.longitude))
                    cleanupSingleLegacyListener(this)
                    cancelTimeout()
                }
    ...
    private fun cleanupSingleLegacyListener(listener: LocationListener) {
        try {
            locationManager.removeUpdates(listener)
        } catch (e: Exception) {
            Log.w("LocationManager", "Failed to remove location updates", e)
        }
        if (singleLegacyListener === listener) {
            singleLegacyListener = null
        }
    }
        ...
        try {
            ...
            fusedLocationClient?.lastLocation?.addOnSuccessListener { location ->
                if (location != null && isLocationRecent(location)) {
                    Log.d(
                        TAG,
                        "Last known fused location: ${location.latitude}, ${location.longitude}"
                    )
                    _userLocation.postValue(GeoPoint(location.latitude, location.longitude))
                }
            }?.addOnFailureListener { e ->
    ...
    private fun startLocationTimeout() {
        ...
        timeoutRunnable = Runnable {
            ...
            singleLegacyListener?.let { cleanupSingleLegacyListener(it) }
