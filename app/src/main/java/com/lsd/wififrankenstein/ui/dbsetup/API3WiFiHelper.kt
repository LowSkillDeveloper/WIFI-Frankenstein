...
class API3WiFiHelper(
    ...
    suspend fun checkMapApiSupport(): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                if (checkTrpcSupport()) {
                    Log.d("API3WiFiHelper", "Detected 3wifi.app (tRPC) protocol")
            ...
            try {
                ...
                Log.d(
                    "API3WiFiHelper",
                    "Map API support check: $isSupported (response: $responseCode)"
                )
    ...
    private suspend fun checkTrpcSupport(): Boolean {
        return withContext(Dispatchers.IO) {
            val params = java.net.URLEncoder.encode("""{"json":{}}""", "UTF-8")
            val testUrl = "$serverUrl/trpc/getAppVersion?input=$params"
            val connection = URL(testUrl).openConnection() as HttpURLConnection
            SslHelper.configure(connection)
            connection.requestMethod = "GET"
            connection.connectTimeout = connectTimeout
                                ...
                                for (i in 0 until networks.length()) {
                                    ...
                                    networkList.add(
                                        mapOf(
                                            ...
                                            "name" to network.optString("name"),
                                            "auth" to network.optString("auth"),
                                            "radioOff" to network.optBoolean("radioOff", false),
                                            "hidden" to network.optBoolean("hidden", false),
                                            "lan_ip" to network.optString("lan_ip"),
                                            "wan_ip" to network.optString("wan_ip"),
                                            "quadkey" to network.optLong("quadkey")
                                        )
                                    )
                                }
