...
class WpsGeneratorAdapter :
    ListAdapter<WpsGeneratorResult, WpsGeneratorAdapter.ViewHolder>(DiffCallback()) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemWpsGeneratorResultBinding.inflate(
    ...
    inner class ViewHolder(private val binding: ItemWpsGeneratorResultBinding) :
        ...
        fun bind(result: WpsGeneratorResult) {
            ...
            binding.textViewPinCount.text =
                itemView.context.getString(R.string.pins_found_count, result.pins.size)

            val suggestedCount = result.pins.count { it.sugg }
            val possibleCount = result.pins.count { !it.sugg && it.showQuestionMark }

            if (suggestedCount > 0) {
                binding.textViewSuggestedCount.visibility = View.VISIBLE
                binding.textViewSuggestedCount.text =
                    itemView.context.getString(R.string.suggested_pins_count, suggestedCount)
            } else {
            ...
            if (possibleCount > 0) {
                ...
                binding.textViewPossibleCount.text =
                    itemView.context.getString(R.string.possible_pins_count, possibleCount)
            } else {
    ...
    inner class PinAdapter : ListAdapter<WPSPin, PinAdapter.PinViewHolder>(PinDiffCallback()) {
        ...
        inner class PinViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
            ...
            private val imageSuggested =
                itemView.findViewById<android.widget.ImageView>(R.id.image_suggested)
            private val imageQuestion =
                itemView.findViewById<android.widget.ImageView>(R.id.image_question)
            private val chipExperimental =
                itemView.findViewById<com.google.android.material.chip.Chip>(R.id.chip_experimental)
            ...
            fun bind(pin: WPSPin) {
                ...
                textAlgo.text = pin.displayName
                imageSuggested.alpha = if (pin.sugg) 1f else 0f
                imageQuestion.alpha = if (pin.showQuestionMark) 1f else 0f
    ...
    class DiffCallback : DiffUtil.ItemCallback<WpsGeneratorResult>() {
        override fun areItemsTheSame(
            oldItem: WpsGeneratorResult,
            newItem: WpsGeneratorResult
        ): Boolean {
