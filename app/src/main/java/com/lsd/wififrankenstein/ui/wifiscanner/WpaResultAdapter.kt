...
class WpaResultAdapter : RecyclerView.Adapter<WpaResultAdapter.WpaResultViewHolder>() {
    ...
    fun submitList(newResults: List<NetworkDatabaseResult>) {
        results =
            newResults.filter { it.resultType == ResultType.WPA_ALGORITHM && it.wpaResult != null }
        notifyDataSetChanged()
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WpaResultViewHolder {
        val binding =
            ItemWpaResultBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    inner class WpaResultViewHolder(private val binding: ItemWpaResultBinding) :
        ...
        private fun copyToClipboard(context: Context, text: String) {
            ...
            Toast.makeText(
                context,
                context.getString(R.string.keys_copied_to_clipboard),
                Toast.LENGTH_SHORT
            ).show()
        }
    }
