...
class ChannelAnalysisAdapter :
    ListAdapter<ChannelAnalysisData, ChannelAnalysisAdapter.ViewHolder>(DiffCallback()) {
    ...
    class ViewHolder(private val binding: ItemChannelAnalysisBinding) :
        ...
        fun bind(data: ChannelAnalysisData, onLongClickListener: ((ChannelAnalysisData) -> Unit)?) {
            ...
            binding.apply {
                ...
                textViewFrequency.text =
                    itemView.context.getString(R.string.frequency_mhz, data.frequency)
                textViewNetworkCount.text =
                    itemView.context.getString(R.string.access_points_count, data.networks.size)

                progressChannelLoad.progress = data.channelLoad
                textViewChannelLoad.text =
                    itemView.context.getString(R.string.channel_load_percent, data.channelLoad)

                textViewStrongSignals.text = itemView.context.getString(
                    R.string.strong_signals_short,
                    data.strongSignalCount
                )
                textViewOverlapping.text = itemView.context.getString(
                    R.string.overlapping_short,
                    data.interferingNetworks.size
                )

                val loadColor = when {
                    data.qualityScore >= 80 -> ContextCompat.getColor(
                        itemView.context,
                        R.color.green_500
                    )

                    data.qualityScore >= 60 -> ContextCompat.getColor(
                        itemView.context,
                        R.color.orange_500
                    )
                ...
                val combinedList = data.networks.map { NetworkListItem.OnChannel(it) } +
                        data.interferingNetworks.map { NetworkListItem.Interfering(it) }
                networkAdapter.submitList(combinedList)
    ...
    private class DiffCallback : DiffUtil.ItemCallback<ChannelAnalysisData>() {
        override fun areItemsTheSame(
            oldItem: ChannelAnalysisData,
            newItem: ChannelAnalysisData
        ): Boolean {
