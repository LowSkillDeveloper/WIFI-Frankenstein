...
class WpsResultAdapter : RecyclerView.Adapter<WpsResultAdapter.WpsResultViewHolder>() {
    ...
    fun submitList(newResults: List<NetworkDatabaseResult>) {
        results =
            newResults.filter { it.resultType == ResultType.WPS_ALGORITHM && it.wpsPin != null }
        notifyDataSetChanged()
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WpsResultViewHolder {
        val binding =
            ItemWpsResultBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    inner class WpsResultViewHolder(private val binding: ItemWpsResultBinding) :
        ...
        private fun copyToClipboard(context: Context, text: String) {
            ...
            Toast.makeText(
                context,
                context.getString(R.string.pin_copied_to_clipboard),
                Toast.LENGTH_SHORT
            ).show()
        }
    }
