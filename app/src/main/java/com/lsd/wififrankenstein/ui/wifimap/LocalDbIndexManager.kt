...
class LocalDbIndexManager(private val context: Context) {
    ...
    suspend fun needsIndexing(db: SQLiteDatabase): Boolean = withContext(Dispatchers.IO) {
        ...
        Log.d(
            TAG,
            "Local database: oldHash=$oldHash, newHash=$newHash, hasIndexes=$hasIndexes, needsIndexing=$needsIndexing"
        )
        needsIndexing
    }
    ...
    private fun hasIndexes(db: SQLiteDatabase): Boolean {
        return try {
            ...
            if (indexLevel == "NONE") return false
            ...
            val placeholders = requiredIndexes.joinToString(",") { "?" }
            db.rawQuery(
                "SELECT COUNT(*) FROM sqlite_master WHERE type='index' AND tbl_name=? AND name IN ($placeholders)",
                arrayOf(LocalAppDbHelper.TABLE_NAME, *requiredIndexes)
            ).use { cursor ->
                cursor.moveToFirst() && cursor.getInt(0) >= requiredIndexes.size
            }
        } catch (e: Exception) {
    ...
    suspend fun createIndexes(
        ...
        try {
            ...
            if (oldHash == newHash && hasIndexes(db)) {
                Log.d(
                    TAG,
                    "Local database hasn't changed and already has indexes, skipping indexing"
                )
                return@withContext true
            }
            ...
            try {
                when (indexLevel) {
                    "FULL" -> {
                        ...
                        db.execSQL("CREATE INDEX IF NOT EXISTS idx_wifi_network_quadkey ON ${LocalAppDbHelper.TABLE_NAME} (${LocalAppDbHelper.COLUMN_QUADKEY})")
                        progressCallback(50)
    ...
    private fun dropIndexes(db: SQLiteDatabase) {
        ...
        db.rawQuery(
            "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name=? AND name != 'idx_wifi_network_quadkey'",
