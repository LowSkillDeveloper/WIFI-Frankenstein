...
class InAppDatabaseViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun updateSearch(
        ...
        searchParameters.value =
            SearchParams(query, filterName, filterMac, filterPassword, filterWps)
    }

    fun checkOnWpaSec(item: WifiNetwork) {
        if (item.macAddress.isBlank() || item.wifiName.isBlank()) return
        viewModelScope.launch(Dispatchers.IO) {
            val app = getApplication<Application>()
            val message = try {
                val found = wpaSecClient.checkPasswordByBssidSsid(
                    wpaSecClient.bssidToHex(item.macAddress),
                    wpaSecClient.essidToHex(item.wifiName)
                )
            ...
            try {
                ...
                getApplication<Application>().contentResolver.openInputStream(uri)
                    ?.use { inputStream ->
                        ...
                        Log.d(
                            "InAppDatabaseViewModel",
                            "JSON Import - Processed: ${stats.totalProcessed}, Inserted: ${stats.inserted}, Duplicates: ${stats.duplicates}"
                        )
                        updateStats()
                    }
                localDbHelper.close()
            ...
            try {
                ...
                getApplication<Application>().contentResolver.openInputStream(uri)
                    ?.use { inputStream ->
                        ...
                        Log.d(
                            "InAppDatabaseViewModel",
                            "CSV Import - Processed: ${stats.totalProcessed}, Inserted: ${stats.inserted}, Duplicates: ${stats.duplicates}"
                        )
                        updateStats()
                    }
                localDbHelper.close()
    ...
    private suspend fun importRecordsWithStats(
        ...
        try {
            val result =
                importRecordsWithStatsLocal(records, importType, progressCallback, localDbHelper)
            localDbHelper.close()
    ...
    fun importFromWpaSec(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val entries = WpaSecImporter.download(apiKey)
