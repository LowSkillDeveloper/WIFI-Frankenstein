...
class UpdatesAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding =
            ItemFileUpdateBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        ...
        fun bind(fileInfo: FileUpdateInfo) {
            binding.apply {
                ...
                textViewLocalVersion.text =
                    root.context.getString(R.string.local_version, fileInfo.localVersion)
                textViewServerVersion.text =
                    root.context.getString(R.string.server_version, fileInfo.serverVersion)
                textViewLocalSize.text =
                    root.context.getString(R.string.local_size, fileInfo.localSize)

                val isDownloading = activeDownloads.contains(fileInfo.fileName)
                val progress = progressMap[fileInfo.fileName] ?: 0
                val isInstalled = fileInfo.localVersion != "0.0" && fileInfo.localVersion != "0"
                ...
                when {
                    ...
                    !isInstalled -> {
                        ...
                        buttonUpdate.text = root.context.getString(R.string.install)
                    ...
                    fileInfo.needsUpdate -> {
                        buttonUpdate.visibility = View.VISIBLE
                        buttonUpdate.isEnabled = true
                        buttonUpdate.text = root.context.getString(R.string.update)
                        buttonUpdate.setOnClickListener { onUpdateClick(fileInfo) }
                        buttonCancel.visibility = View.GONE
                        buttonRevert.visibility = View.GONE
                        progressBarUpdate.visibility = View.GONE
