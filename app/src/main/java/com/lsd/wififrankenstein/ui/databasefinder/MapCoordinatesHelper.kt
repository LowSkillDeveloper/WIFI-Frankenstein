...
class MapCoordinatesHelper(private val context: Context) {
    ...
    suspend fun getCoordinates(bssid: String, dbItem: DbItem): Pair<Double?, Double?> =
        withContext(Dispatchers.IO) {
            try {
                ...
                while (attempts < maxAttempts && result == null) {
                    ...
                    Log.d(
                        TAG,
                        "Attempt $attempts to get coordinates for BSSID: $bssid from: ${dbItem.path}"
                    )

                    Log.d(TAG, "getCoordinates: bssid=$bssid, dbType=${dbItem.dbType}")

                    result = when (dbItem.dbType) {
                        DbType.SQLITE_FILE_P3WIFI, DbType.SMARTLINK_SQLITE_FILE_P3WIFI ->
                            getCoordinatesFrom3WiFiDb(bssid, dbItem)

                        DbType.SQLITE_FILE_CUSTOM, DbType.SMARTLINK_SQLITE_FILE_CUSTOM ->
                            getCoordinatesFromCustomDb(bssid, dbItem)
                    ...
                    if (result.first != null && result.second != null) {
                        Log.d(
                            TAG,
                            "Found coordinates on attempt $attempts: lat=${result.first}, lon=${result.second}"
                        )
                        break
                    } else if (attempts < maxAttempts) {
                        Log.d(
                            TAG,
                            "No coordinates found on attempt $attempts, waiting for next attempt"
                        )
                        delay(600)
                    }
                }
    ...
    private suspend fun getCoordinatesFrom3WiFiDb(
        bssid: String,
        dbItem: DbItem
    ...
    private suspend fun getCoordinatesFromCustomDb(
        bssid: String,
        dbItem: DbItem
    ): Pair<Double?, Double?> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun getLocalAppDbCoordinates(bssid: String): Pair<Double?, Double?> =
        getCoordinatesFromLocalDb(bssid)

    private suspend fun getCoordinatesFromLocalDb(bssid: String): Pair<Double?, Double?> =
        withContext(Dispatchers.IO) {
            try {
            ...
            """.trimIndent()
                ...
                helper.readableDatabase.rawQuery(
                    query,
                    arrayOf(bssid, "%$bssid%", cleanMac.uppercase())
                ).use { cursor ->
                    if (cursor.moveToFirst()) {
