...
object VendorChecker {
    ...
    private fun getFileFromInternalStorageOrAssets(context: Context, fileName: String): File {
        ...
        if (!file.exists()) {
            try {
                ...
                createVersionFile(context, fileName, if (fileName == "vendor.db") "1.2" else "1.0")
    ...
    suspend fun checkVendorLocalSource1(context: Context, bssid: String): String =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                Log.d(
                    "VendorChecker",
                    "Using vendor.db version: $version from path: ${dbFile.absolutePath}"
                )

                SQLiteDatabase.openDatabase(dbFile.path, null, SQLiteDatabase.OPEN_READONLY)
                    .use { localDB ->
                        localDB.rawQuery("SELECT vendor FROM oui WHERE mac = ?", arrayOf(bssid))
                            .use { cursor ->
                                if (cursor.moveToFirst()) {
                                    ...
                                    Log.d(
                                        "VendorChecker",
                                        "Found vendor: $vendor for BSSID: $bssid"
                                    )
                                    vendor
                                } else {
    ...
    suspend fun checkVendorOnlineSource2(
        context: Context,
        bssid: String,
        dbList: List<DbItem>
    ): String {
                ...
                try {
                    ...
                    if (isAlternativeUrlEnabled(context)) {
                        try {
                            Log.d(
                                "VendorChecker",
                                "Sending GET request to alternative URL: $alternativeUrl"
                            )
                            val altResponse =
                                Jsoup.connect(alternativeUrl).ignoreContentType(true).execute()
                                    .body()
                            ...
                            if (altJson.getBoolean("result")) {
                                ...
                                Log.d(
                                    "VendorChecker",
                                    "BSSID data array length: ${bssidData.length()}"
                                )
                            ...
                            Log.e(
                                "VendorChecker",
                                "Error checking vendor for BSSID $bssid with alternative URL",
                                altException
                            )
                        }
                    }
