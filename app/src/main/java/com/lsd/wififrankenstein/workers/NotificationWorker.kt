...
class NotificationWorker(
    ...
    private val prefs =
        applicationContext.getSharedPreferences("notification_worker", Context.MODE_PRIVATE)

    companion object {
        private const val GENERAL_NOTIFICATIONS_URL =
            "https://raw.githubusercontent.com/LowSkillDeveloper/WIFI-Frankenstein/refs/heads/service/notifications.json"
    ...
    private suspend fun checkForUpdates() {
        ...
        updateStatus.appUpdate?.let { appUpdate ->
            val currentVersion = applicationContext.packageManager.getPackageInfo(
    ...
    private suspend fun checkGeneralNotifications() {
        try {
            ...
            val notificationsResponse =
                json.decodeFromString<GeneralNotificationsResponse>(response)

            val shownNotifications =
                prefs.getStringSet(KEY_SHOWN_NOTIFICATIONS, emptySet()) ?: emptySet()
            ...
            notificationsResponse.notifications.forEach { notification ->
                if (notification.id !in shownNotifications) {
                    ...
                    val isVersionMatch = isVersionInRange(
                        currentVersion,
                        notification.min_version,
                        notification.max_version
                    )
            ...
            com.lsd.wififrankenstein.util.Log.e(
                "NotificationWorker",
                "Error checking general notifications",
                e
            )
        }
    }

    private fun isVersionInRange(
