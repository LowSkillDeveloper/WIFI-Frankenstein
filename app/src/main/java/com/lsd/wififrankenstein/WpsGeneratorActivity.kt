...
class WpsGeneratorActivity : AppCompatActivity() {
    ...
    private fun applyCurrentTheme() {
        ...
        val colorTheme = prefs.getString("color_theme", "green") ?: "green"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        contentBinding =
            ContentWpsGeneratorBinding.bind(binding.root.findViewById(R.id.content_wps_generator))
        ...
        pinListAdapter.targetBssid = bssid
    ...
    private suspend fun searchMultiLevelNeighbors(bssid: String, dbItem: DbItem): List<WPSPin> {
        ...
        allPins.addAll(
            searchNeighborsByDistance(
                bssid,
                dbItem,
                10,
                1.2,
                getString(R.string.very_close_neighbor)
            )
        )
        ...
        allPins.addAll(
            searchNeighborsByDistance(
                ...
                100,
                1.0,
                getString(R.string.close_neighbor)
            )
        )
        ...
        if (allPins.size < 10) {
            allPins.addAll(
                searchNeighborsByDistance(
                    ...
                    1000,
                    0.7,
                    getString(R.string.medium_neighbor)
                )
            )
        }
                ...
                helper.database?.rawQuery(
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        if (isValidWpsPin(wpsPin)) {
                            val distance =
                                kotlin.math.abs((targetNic - (neighborDecimal and 0xFFFFFF)).toInt())
                            if (distance >= minDistance && distance <= maxDistance) {
                                val score = calculateNeighborScore(
                                    targetDecimal,
                                    neighborDecimal
                                ) * weightMultiplier
                                totalScore += score
                ...
                try {
                    when (dbItem.dbType) {
                        ...
                        DbType.LOCAL_APP_DB -> {
                            ...
                            Log.d(
                                "WpsGeneratorActivity",
                                "Skipping database type: ${dbItem.dbType}"
                            )
                        }
                    }
                } catch (e: Exception) {
    ...
    private fun setupWebView() {
        ...
        contentBinding.webView.loadDataWithBaseURL(
            null,
            modifiedHtmlContent,
            "text/html",
            "UTF-8",
    ...
    private fun generatePinsUsingNewGenerator() {
        ...
        val sortedPins =
            wpsPins.sortedWith(compareByDescending<WPSPin> { it.sugg }.thenBy { it.isExperimental })
                        ...
                        if (jsonResponse.getBoolean("result")) {
                            ...
                            if (data != null && data.has(uppercaseBssid)) {
                                ...
                                if (serverPins.isNotEmpty()) {
                                    ...
                                    Log.w(
                                        "WpsGeneratorActivity",
                                        "No pins found for BSSID: $uppercaseBssid on server: ${server.path}"
                                    )
                                }
                            } else {
                                Log.w(
                                    "WpsGeneratorActivity",
                                    "No data found for BSSID: $uppercaseBssid on server: ${server.path}"
                                )
                            }
                        } else {
        ...
        fun getPins(all: Int, json: String?, bssid: String) {
            try {
                ...
                for (i in 0 until arr.length()) {
                    ...
                    val name =
                        if (algoIndex != -1 && algoIndex < algos.size) algos[algoIndex].name else "Unknown"
                    val mode =
                        if (algoIndex != -1 && algoIndex < algos.size) algos[algoIndex].mode else 0
