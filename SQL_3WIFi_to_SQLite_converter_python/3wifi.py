...
def show_mode_menu():
    menu = """
╔═══════════════════════════════════════════════════════════════════════════════════════════════════╗
║                           Anti3WiFi & 3WiFi Database Converter v4                                 ║
...
║     • Processing time may increase by 3–4 times                                                   ║
...
def stream_sql_file(sql_file, chunk_size=1024*1024):
...
def extract_insert_statements_memory_efficient(sql_file, table_name):
    ...
    patterns = [
        rf'INSERT\s+IGNORE\s+INTO\s+`?{re.escape(table_name)}`?\s*\([^)]+\)\s*VALUES\s*',
        rf'INSERT\s+INTO\s+`?{re.escape(table_name)}`?\s*\([^)]+\)\s*VALUES\s*',
        rf'INSERT\s+IGNORE\s+INTO\s+`?{re.escape(table_name)}`?\s+VALUES\s*',
    ...
    with tqdm(total=file_size, desc=f"Scanning {table_name}", unit="B", unit_scale=True) as pbar:
        for chunk in stream_sql_file(sql_file):
            ...
            for line in lines[:-1]:
                ...
                if in_insert_block and (line.endswith(';') or 'INSERT INTO' in line.upper() or 'INSERT IGNORE INTO' in line.upper()):
...
def main():
    ...
    print("╔═══════════════════════════════════════════════════════════════════════════════════════════════════╗")
    print("║                           Anti3WiFi & 3WiFi Database Converter v4                                 ║")
    ...
    while True:
        ...
        if memory_efficient_mode:
            records_geo, errors_geo = execute_inserts_memory_efficient(db_file, geo_file, 'geo', 4, error_file_geo)
            records_base, errors_base = execute_inserts_memory_efficient(db_file, base_file, 'base', 23, error_file_base)
        else:
